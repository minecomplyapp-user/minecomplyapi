...
import { CMVRDocxGeneratorService } from '../src/cmvr/cmvr-docx-generator.service';
import type { CMVRGeneralInfo } from '../src/cmvr/cmvr-pdf-generator.service';

console.log('[docx-preview] Starting script...');

function loadDataOrSample(): CMVRGeneralInfo {
  const cliArgPath = process.argv[2] ? resolve(process.argv[2]) : undefined;
  const defaultPath = join(__dirname, '..', 'data', 'cmvr-report.json');

  const tryPath =
    cliArgPath && existsSync(cliArgPath)
      ? cliArgPath
  ...
  if (tryPath) {
    try {
      ...
      const parsed = JSON.parse(raw) as CMVRGeneralInfo;
      console.log(`[docx-preview] Using data from: ${tryPath}`);
      return parsed;
    } catch (e) {
      console.warn(
        '[docx-preview] Failed to parse JSON data, falling back to sample. Error:',
        e,
      );
    }
  } else if (cliArgPath) {
    console.warn(
      `[docx-preview] Provided data path not found: ${cliArgPath}. Falling back to sample.`,
    );
  }
      ...
      implementationOfEnvironmentalImpactControlStrategies: [
        ...
        },
        {
          ...
          commitments: [
            ...
            {
              plannedMeasure:
                '8. Segregate recyclable for non-recyclable waste',
              actualObservation:
...
async function main() {
  const service = new CMVRDocxGeneratorService();

  const data = loadDataOrSample();
  console.log('Generating CMVR DOCX preview (full report)...');
  const buffer = await service.generateFullReportDocx(data);

  const outPath = join(__dirname, '..', 'tmp', 'cmvr-preview.docx');
